/**
  ...
  *			
	...
	*				任务9：app_task9_serve			服务器器
	*				任务10：app_task10_mq7		co报警器
	*				任务11:	app_task11_serve 	服务器
	*				任务12：app_task12_esp8266 wifi管理+时间+天气
	*				任务13: app_task13_weather	天气
	*				事件标志组：
	*								event1_group_handle（esp8266->weather(BIT_0),WEATHER->serve(BIT_1))
	*				互斥锁：
	*								esp_sem_mutex_handle;（esp8266相关任务)
	* 							esp_u3_recvbuf_sem_mutex_handle;(保护u3_recvbuf)
	*				消息队列：
	*								msg_queue_handle (rtc->lvgl;adc->lvgl，dht11->lvgl,mq2->lvgl,mq7->lvgl,mq135->lvgl)
	*								time_queue_handle(esp8266time->rtc)
	*               weather_queue_handle(
	*               bafa_queue_handle(lvgl->bafayun)
  *
  ******************************************************************************
  * @attention
  *
...
uint32_t bemfa_last_heartbeat = 0;  			//心跳
...
TaskHandle_t app_task11_serve_handle	    = NULL;
...
static void app_task11_serve(void *pvParameters);
...
// 互斥信号量
static SemaphoreHandle_t esp_sem_mutex_handle;
SemaphoreHandle_t	esp_u3_recvbuf_sem_mutex_handle;
//消息队列
...
static QueueHandle_t bafa_queue_handle=NULL;
// 声明一个自定义的 TTF 字体
...
//	// 三、创建事件标志组
	...
	//创建互斥信号量
	esp_sem_mutex_handle = xSemaphoreCreateMutex();
	esp_u3_recvbuf_sem_mutex_handle = xSemaphoreCreateMutex();
	 // 三、创建消息队列、  
   msg_queue_handle  = xQueueCreate(QUEUE_LEN, sizeof(MsgStructDef)); 
	...
	bafa_queue_handle  = xQueueCreate(QUEUE_LEN, sizeof(MsgStructDef)); 
	/* 创建app_task_init任务 */
...
//	IIC_Init();
	...
	xReturned = xTaskCreate(app_task3_lvgl,"app_task3_lvgl", 640, NULL,  7, &app_task3_lvgl_handle);     
	if( xReturned == pdPASS )
    {
	...
	if( xReturned == pdPASS )
    {
		...
		// 4、创建app_task11_serve 服务器任务
		xReturned = xTaskCreate(app_task11_serve,"app_task11_serve", 640, NULL, 4, &app_task11_serve_handle);
		if( xReturned == pdPASS ) printf("app_task11_serve create success!\r\n");

		// 4、创建app_task12_esp8266 wifi加时间
		xReturned = xTaskCreate(app_task12_esp8266,"app_task12_esp8266", 640, NULL, 6, &app_task12_esp8266_handle);
		if( xReturned == pdPASS ) printf("app_task12_esp8266 create success!\r\n");

		// 4、创建app_task13_weather 加天气
		xReturned = xTaskCreate(app_task13_weather,"app_task13_weather", 640, NULL, 5, &app_task13_weather_handle);
		if( xReturned == pdPASS ) printf("app_task13_weather create success!\r\n");
...
static void app_task3_lvgl(void *pvParameters)
{
    ...
    for (;;)
    {
		...
		//	{
			if(xQueueReceive(msg_queue_handle,&recv_msg,0)==pdPASS)
			{
				...
				if(recv_msg.type==msg_type_rtc)
				{
				  ...
				  lv_label_set_text(wendu, dht11_wendu);
					...
					xQueueSend(bafa_queue_handle, &recv_msg, pdMS_TO_TICKS(100));
			...
			{
					if(weather_msg.sendweather == msg_type_weather)   
					{
							char weather_buf[64];
...
static void app_task11_serve(void *pvParameters)
{
		//char wenshidu_buf[32]={0};
		char qita_buf[32]={0};
    printf("巴法云任务启动... 等待天气同步完成\r\n");
    MsgStructDef recv_msg;
    // 等待天气任务完成
    xEventGroupWaitBits(event1_group_handle, BIT_1, pdTRUE, pdTRUE, portMAX_DELAY);
    printf("天气同步完成，开始连接巴法云...\r\n");

    // 初始连接
    if (xSemaphoreTake(esp_sem_mutex_handle, pdMS_TO_TICKS(10000)) == pdTRUE)
    {
        if (ESP8266_BemFaYun_Tcp_Connect() == 0)
        {
        ...
        xSemaphoreGive(esp_sem_mutex_handle);
    }

    for (;;)
    {
        if (xTaskGetTickCount() - bemfa_last_heartbeat > pdMS_TO_TICKS(60000))
        {
            if (xSemaphoreTake(esp_sem_mutex_handle, pdMS_TO_TICKS(5000)) == pdTRUE)
            {
                BemFaYun_SendHeartbeat();
                bemfa_last_heartbeat = xTaskGetTickCount();
                xSemaphoreGive(esp_sem_mutex_handle);
            }
        }

        if (u3_flag || strstr((char*)u3_recvbuf, "cmd=3") != NULL)
        {
            printf("【巴法云下发】%s\r\n", (char*)u3_recvbuf);
            ...
            u3_count = u3_len = u3_flag = 0;
				...
				if(xQueueReceive(bafa_queue_handle,&recv_msg,0)==pdPASS)
			  {
				if(recv_msg.type==msg_type_dht11)
		   	{
				// ========== 1 发 温湿度 ==========
				sprintf(qita_buf, "#%d#%d", recv_msg.wenshidu[2], recv_msg.wenshidu[0]);
				ESP8266_BemFaYun_Tcp_Publish(BEMFAYUN_SIYAO, "wenshidu111", qita_buf);
				memset(qita_buf,0,sizeof(qita_buf));					
				}
				if (recv_msg.type == msg_type_mq2)
				{
				// ========== 2 发 烟雾 ==========
				sprintf(qita_buf, "#%.2f",recv_msg.mq2_num);
				ESP8266_BemFaYun_Tcp_Publish(BEMFAYUN_SIYAO, "yanwu111",qita_buf);
				memset(qita_buf,0,sizeof(qita_buf));		
				}
				if (recv_msg.type == msg_type_mq7)
				{
				// ========== 3 发 CO ==========
				sprintf(qita_buf, "#%.2f", recv_msg.mq7_num);
				ESP8266_BemFaYun_Tcp_Publish(BEMFAYUN_SIYAO, "CO111", qita_buf);
				memset(qita_buf,0,sizeof(qita_buf));		
				}
					if (recv_msg.type == msg_type_mq135)
					{
				// ========== 4 发 空气质量 ==========
						sprintf(qita_buf, "#%d",recv_msg.mq135_num);
				ESP8266_BemFaYun_Tcp_Publish(BEMFAYUN_SIYAO, "zhiliang111", qita_buf);
				memset(qita_buf,0,sizeof(qita_buf));		
					...
					{
				// ========== 5 发 光照 ==========
						sprintf(qita_buf, "#%d",recv_msg.adc_val);
				ESP8266_BemFaYun_Tcp_Publish(BEMFAYUN_SIYAO, "guangzhao111",qita_buf);
				memset(qita_buf,0,sizeof(qita_buf));		
						}
...
static void app_task12_esp8266(void *pvParameters)
{
    ...
    // 初始联网
    if (xSemaphoreTake(esp_sem_mutex_handle, pdMS_TO_TICKS(15000)) == pdTRUE)
    {
    ...
    for (;;)
    {
        if (xSemaphoreTake(esp_sem_mutex_handle, portMAX_DELAY) == pdTRUE)
        {
            memset(&msg, 0, sizeof(time_struct));
            msg.sendtime = msg_type_esp8266_time;

           int8_t ret=ESP8266_GetTimeFunc(&msg.esp_time);
					if(ret==0)
					{
						printf("yes\r\n");
					}
...
//						printf("Min    : %s\r\n", msg.esp_time.min);
//						printf("Second : %s\r\n", msg.esp_time.second);
						xEventGroupSetBits(event1_group_handle, BIT_0);   // 通知其他任务可以启动
...
static void app_task13_weather(void *pvParameters)
{
    ...
    printf("【Weather】等待时间同步...\r\n");
    xEventGroupWaitBits(event1_group_handle, BIT_0, pdFALSE, pdTRUE, portMAX_DELAY);  // pdFALSE = 不清除标志
    printf("【Weather】时间同步完成，开始周期获取天气...\r\n");

    for (;;)
    {
        if (xSemaphoreTake(esp_sem_mutex_handle, pdMS_TO_TICKS(6000)) == pdTRUE)
        {
...
//			
        ...
        while (retry_count < 2)
        {
            printf("【Weather】第 %d 次尝试获取天气...\r\n", retry_count + 1);
            
            if (ESP8266_GetWeatherFunc(XZTQ_KEY, "guangzhou") == 0)
            {
                // 成功
								strcpy(msg.weather_text, results[0].now.text);
                strcpy(msg.weather_name, results[0].location.name);
                ...
                printf("天气更新成功: %s - %s\r\n", msg.weather_name, msg.weather_text);
                
                xQueueSend(weather_queue_handle, &msg, pdMS_TO_TICKS(100));
                xEventGroupSetBits(event1_group_handle, BIT_1);
                break;                    // 成功后跳出重试循环
            }
            else
            {
                retry_count++;
                printf("第 %d 次获取失败，%d秒后重试...\r\n", retry_count, 2);
                vTaskDelay(pdMS_TO_TICKS(2000));   // 失败后等待2秒再重试
            }
        }
        ...
        vTaskDelay(pdMS_TO_TICKS(600000));   // 10分钟一次
    }
}
