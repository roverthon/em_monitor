...
-----------------------------------------------------------------------
*/
void lcd_send_data(vu16 data)
{

	LCD_WR_DATA(data);

}
void lcd_addr_set(uint16_t x_s, uint16_t y_s, uint16_t x_e, uint16_t y_e)
{
	uint16_t width =x_e-x_s+1;
	uint16_t height=y_e-y_s+1;	
	
	//设置显示区域
	LCD_Set_Window(x_s,y_s,width,height);
	
	//开始写入GRAM
	LCD_WriteRAM_Prepare();	   
}

void lcd_show_char(uint16_t x, uint16_t y,uint16_t no, uint16_t fc, uint16_t bc,uint16_t font_size)
{
	uint32_t i,j;
	uint8_t tmp;
	...
	{
//		if(font_size==16)tmp = g_font_dot_matrix_16[no][i];
//		if(font_size==24)tmp = g_font_dot_matrix_24[no][i];	
		if(font_size==32)tmp = g_font_dot_matrix_32[no][i];
...
int FindFontIndex(const char *target,uint32_t font_size)
{
	int i;
	...
	if(font_size == 32)
	{
		...
		for (i = 0; i < (sizeof(g_font_dot_matrix_32_index) / sizeof(g_font_dot_matrix_32_index[0])); i++)
		{
			...
			if (strncmp(g_font_dot_matrix_32_index[i], target, (target[0] > 0x7F) ? 2 : 1) == 0)
			{
				// 返回索引值
				return i;
			}
...
void lcd_show_string(uint32_t x, uint32_t y,const char *str, uint32_t fc, uint32_t bc,uint32_t font_size)
{
	...
	{
		// 获取索引值
		j = FindFontIndex((const char *)&str[i],font_size);
		...
		// 显示文字（支持普通汉字和ASC）
		lcd_show_char(x, y, j,fc,bc,font_size);
...
void lcd_draw_image(uint16_t x_s, uint16_t y_s, uint16_t width, uint16_t height, const uint8_t *image)
{
...
void LCD_BackLight(uint8_t sta)
{
    if(sta)
    {
        GPIO_SetBits(GPIOB, GPIO_Pin_15);  // PB15输出高电平，亮屏
    }
    else
    {
        GPIO_ResetBits(GPIOB, GPIO_Pin_15);// PB15输出低电平，熄屏
    }
}