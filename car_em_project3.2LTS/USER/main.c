...
//	IIC_Init();
	...
	xReturned = xTaskCreate(app_task6_dht11,"app_task6_dht11", 128, NULL,  6, &app_task6_dht11_handle);     
	if( xReturned == pdPASS )
    {
...
////			
////				printf("##### 获取天气中成功 #####\r\n");
////				strcpy(msg.weather.location.name,results[0].location.name);
////				strcpy(msg.weather.now.text,results[0].now.text);
////				// 释放锁
////				printf("城市：%s\n", msg.weather.location.name);
////				printf("天气：%s\n", msg.weather.now.text);
////				xSemaphoreGive(esptime_weather_sem_mutex_handle);	
////			
//				vTaskDelay(pdMS_TO_TICKS(1000));
//    }
