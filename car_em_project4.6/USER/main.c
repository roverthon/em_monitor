...
//	IIC_Init();
	...
	xReturned = xTaskCreate(app_task11_serve,"app_task11_serve", 256, NULL,  4, &app_task11_serve_handle);     
	if( xReturned == pdPASS )
    {
		printf("app_task11_serve create success!\r\n");
    }
...
static void app_task11_serve(void *pvParameters)
{
	...
	char     buf[32] 	  = {0};
		...
		// 2、连接巴法云服务器，并向其订阅主题
	    // 加锁保护整个巴法云连接和订阅过程
    if (xSemaphoreTake(esptime_weather_sem_mutex_handle, portMAX_DELAY) == pdTRUE)
    {
	ESP8266_BemFaYun_Tcp_Connect();
	printf("##### 连接巴法云服务器成功 #####\r\n");
...
static void app_task13_weather(void *pvParameters)
{
    ...
    for (;;) {
        ...
        xEventGroupWaitBits(event1_group_handle, BIT_0, pdFALSE, pdFALSE, pdMS_TO_TICKS(3000));
